...
const AssignmentForm = ({ tour, existingAssignments, onSuccess, onClose }) => {
  ...
  const fetchGuides = async () => {
    try {
      setLoading(true);
      const res = await guideApi.getForTour(tour.id);
      setGuides(res.data || []);
    } catch (err) {
  ...
  const handleRoleChange = (guideId, role) => {
    setSelectedGuides((prev) =>
      prev.map((g) => (g.guideId === guideId ? { ...g, role } : g)),
    );
  };

  const handleNoteChange = (guideId, note) => {
    setSelectedGuides((prev) =>
      prev.map((g) => (g.guideId === guideId ? { ...g, note } : g)),
    );
  };
  ...
  const isSelected = (guideId) =>
    selectedGuides.some((g) => g.guideId === guideId);
        ...
        >
          <div>
            ...
            <p style={{ margin: "4px 0 0", opacity: 0.85, fontSize: "14px" }}>
              Tour: <strong>{tour.name}</strong> ({tour.code}) | Cần tối thiểu:{" "}
              {tour.minGuides || 1} HDV
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          {selectedGuides.length > 0 && (
            ...
            >
              ...
              {selectedGuides.map((g) => (
                ...
                >
                  <span style={{ fontWeight: 600, color: "#1e40af", flex: 1 }}>
                    {g.guideName}
                  </span>
                  ...
                  >
                    ...
                    <select
                      value={g.role}
                      onChange={(e) =>
                        handleRoleChange(g.guideId, e.target.value)
                      }
                      style={{
                        padding: "4px 8px",
                        borderRadius: "6px",
                  ...
                  <input
                    ...
                    value={g.note}
                    onChange={(e) =>
                      handleNoteChange(g.guideId, e.target.value)
                    }
                    style={{
                      padding: "4px 8px",
                      borderRadius: "6px",
                  ...
                  <button
                    onClick={() =>
                      setSelectedGuides((prev) =>
                        prev.filter((x) => x.guideId !== g.guideId),
                      )
                    }
                    style={{
                      background: "#fee2e2",
        ...
        >
          ...
          <button
            ...
            style={{
              padding: "10px 24px",
              background:
                saving || selectedGuides.length === 0 ? "#9ca3af" : "#1d4ed8",
              color: "#fff",
              border: "none",
              borderRadius: "8px",
              cursor:
                saving || selectedGuides.length === 0
                  ? "not-allowed"
                  : "pointer",
              fontSize: "14px",
              fontWeight: 700,
          ...
          >
            {saving
              ? "Đang lưu..."
              : `Lưu phân công (${selectedGuides.length})`}
          </button>
        </div>
      </div>
    </div>

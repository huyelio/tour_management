...
const ItineraryList = ({ tourId, durationDays, tourStatus }) => {
  ...
  const handleDelete = async (item) => {
    if (!window.confirm(`Bạn có chắc muốn xóa hoạt động "${item.title}"?`))
      return;
    try {
  ...
  return (
    ...
    >
      ...
      {!canEdit && (
        ...
        >
          Tour đã {tourStatus === "COMPLETED" ? "hoàn thành" : "bị hủy"} — không
          thể chỉnh sửa lịch trình
        </div>
      )}
              ...
              {grouped[day].map((item, idx) => (
                <div
                  ...
                  style={{
                    ...
                    borderLeft:
                      "3px solid " + (item.isOptional ? "#d1d5db" : "#2563eb"),
                  }}
                >
